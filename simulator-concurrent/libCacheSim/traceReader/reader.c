...
reader_t *setup_reader(const char *const trace_path,
  ...
  char* text = "thisisnowadummytexttrac";
  char* trace_path_d= "dummy.txt";

  fd = open(trace_path_d, O_RDONLY);
  if (fd < 0) {
      // File doesn't exist or can't be opened for reading — create it
      FILE *f = fopen(trace_path_d, "w");
      if (!f) {
          perror("fopen");
          return NULL;
      }
