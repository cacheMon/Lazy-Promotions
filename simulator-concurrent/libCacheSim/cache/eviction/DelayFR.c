...
static cache_obj_t *DelayFR_find(cache_t *cache, const request_t *req, const bool update_cache) {
  ...
  if (obj != NULL && update_cache) {
    int64_t current_time = atomic_load(&params->current_time);
    int64_t obj_last_access = atomic_load(&obj->delay_FR.last_reuse_time);
    int64_t time_passed = current_time - obj_last_access;
...
static cache_obj_t *DelayFR_insert(cache_t *cache, const request_t *req) {
  ...
  if (obj != NULL) {
    ...
    if (!cache->warmup_complete) {
      ...
      atomic_fetch_add(&params->current_time, 1);
      atomic_store(&obj->clock.freq, 0);
      atomic_store(&obj->delay_FR.last_reuse_time, atomic_load(&params->current_time));
    }
  }
#ifdef USE_BELADY
  obj->next_access_vtime = req->next_access_vtime;
